            ...
            <div class="flex-1">
                <div class="col-3 relative">
                    ...
                    <draggable
                        ...
                        class="list-group grid grid-cols-3 gap-3" :list="list2" group="people" @change="log"
                        itemKey="name">
                        <template #item="{ element, index }">
                            <div class="list-group-item p-4" v-if="!element.data">{{ element.name }} {{ index }}</div>
                            <div class="list-group-item p-4 cursor-move" v-else>
                                <button @click="updateData(index)"
                                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                    yangilash

                                </button>
                                <highchart :id="index" :options="element.data"  />
                            </div>
                        </template>
                    </draggable>
